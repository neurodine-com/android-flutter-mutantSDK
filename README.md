...
> This is a successfull attempt to develope and build in VSCode a Flutter (dart) project.
> Idea is to create a Application (game) using a framework that runs smoothly on Android, Windows, Linux, WebBr, MacOS, iOS.


use androidSDK without **/dev/kvm** capable computer and reuse dockered androidsdk-31 data.
...
***1.PC's BIOS **no KVM**|**no VT* support:
...
[^saveresourcespc]:
  Linux Mint: SSD:256GB, RAM:12GB DDR, CPU baaad
  ```Bash grep -iE '^((model name)|(vendor_id))[[:blank:]]+:.*$' < <(</proc/cpuinfo)```
